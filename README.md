...
-- This field requires a specific pattern of colors to be matched. Some handlers
...
The `register()` method must return a dictionary of groups that will be handled along with the constructor of the class. If a registered handler matches the group name, then the corresponding handler class is constructed, setting the required parameters. Then, the `apply()` method is called after construction. Handlers which do not implement a registration method will be ignored.
...
> [!NOTE]
> The `chroma.colors` API will be very unstable during development, as I'm not happy with where it is right now. Be warned.
...
The main benefit of using `Color` over a string representation is that casting to other color types, like RGB or HSL, is trivial. Moreover, the colors can be easily manipulated. Look at the `imagemagick` backend for color generation. It extensively uses the colorspace casting and color modification, especially in color generation. 

> [!IMPORTANT]
> Currently, only the RGB, HSL, and Hexadecimal color spaces are supported. Kind of going against the design philosophy of Chroma, it is not easily extendable, unfortunately. This is something that needs to be worked upon.
...
new_color.normalize()  # only supported by ColorHSL and ColorRGB.
